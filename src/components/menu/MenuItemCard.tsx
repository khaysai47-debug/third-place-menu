...
export function MenuItemCard({ item, variant = "compact", onAdd }: Props) {
  if (variant === "feature") {
    return (
      <article className="relative paper-grain rounded-2xl border border-[var(--color-gold)]/30 overflow-hidden shadow-[0_24px_50px_-30px_oklch(0_0_0/0.9)]">
          ...
          <div className="col-span-3 p-4 flex flex-col">
            <div className="flex items-start gap-2">
              ...
              <div className="flex flex-wrap gap-1">
                {item.tags?.slice(0, 2).map((t) => (
                  <span key={t} className={`text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded-sm border ${tagColor(t)}`}>{t}</span>
                ))}
              </div>
            </div>
            ...
            <div className="mt-auto pt-3 flex items-end justify-between">
              <div>
                ...
                <p className="text-[10px] uppercase tracking-wider text-[var(--color-ink)]/60 mt-0.5">{item.unit}</p>
              </div>
  ...
  if (variant === "row") {
    ...
    return (
      <article className="paper-grain rounded-xl border border-[var(--color-gold)]/25 px-3.5 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline justify-between gap-3">
            <h4 className="font-display text-[16px] text-[var(--color-ink)] truncate">{item.nameEn}</h4>
          ...
          <div className="mt-0.5 flex items-center gap-2 text-[11px] text-[var(--color-ink)]/65">
            ...
            {item.tags?.slice(0, 2).map((t) => (
              <span key={t} className={`px-1.5 py-px rounded-sm border text-[10px] ${tagColor(t)}`}>{t}</span>
            ))}
          </div>
        </div>
        <AddButton onClick={() => onAdd(item)} />
  ...
  // compact
  return (
    <article className="paper-grain rounded-xl border border-[var(--color-gold)]/30 overflow-hidden">
      <div className="flex">
        ...
        <div className="flex-1 p-3 flex flex-col">
          <div className="flex items-start justify-between gap-2">
            <h4 className="font-display text-[16px] leading-tight text-[var(--color-ink)]">{item.nameEn}</h4>
            {item.popular && (
              <span className="text-[9px] uppercase tracking-[0.18em] bg-[var(--color-vermillion)] text-[var(--color-cream)] px-1.5 py-0.5 rounded-sm">★</span>
            )}
          </div>
          <p className="mt-1 text-[12px] leading-snug text-[var(--color-ink)]/65 line-clamp-2">{item.descriptionEn}</p>
          <div className="mt-auto pt-2 flex items-end justify-between">
            <div className="flex flex-col">
              <Price value={item.price} />
              <span className="text-[10px] uppercase tracking-wider text-[var(--color-ink)]/55">{item.unit}</span>
            </div>
